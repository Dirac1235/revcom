...
import {
  Plus, Edit, Eye, Trash2, FileText, MessageSquare,
  DollarSign, Calendar, Tag, TrendingUp, Clock, CheckCircle
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium capitalize ${config.className}`}>
      {config.icon}
      {config.label}
    </span>
  );
...
function SummaryStats({ requests }: { requests: any[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
      {[
        { label: "Total", value: requests.length, color: "text-foreground", bg: "bg-secondary/60" },
        { label: "Open", value: open, color: "text-green-700 dark:text-green-400", bg: "bg-green-50 dark:bg-green-900/10" },
        { label: "Negotiating", value: negotiating, color: "text-yellow-700 dark:text-yellow-400", bg: "bg-yellow-50 dark:bg-yellow-900/10" },
        { label: "Closed", value: closed, color: "text-gray-500", bg: "bg-gray-50 dark:bg-gray-800/40" },
      ].map((stat) => (
...
function RequestCard({ request }: { request: any }) {
  ...
  return (
    <Card className="border-border shadow-none rounded-2xl hover:shadow-sm transition-all group">
...
function EmptyState() {
  return (
    <Card className="border-dashed border-border shadow-none rounded-2xl bg-transparent">
      <CardContent className="py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-6 max-w-xs mx-auto">
          Post your first request and let sellers come to you with competitive offers.
        </p>
    ...
    <div className="min-h-screen bg-background">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex justify-between items-start mb-8 gap-4">
          ...
          <Link href="/buyer/requests/create">
            <Button className="bg-foreground text-background hover:bg-foreground/90 shadow-none gap-2 flex-shrink-0">
              <Plus className="w-4 h-4" />
              New Request
            </Button>
          </Link>
