  ...
  return (
    <div className="min-h-screen bg-background pb-16">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {allList.length > 0 && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            {[
              ...
              <Card
                key={stat.label}
                className={`border shadow-sm ${stat.urgent && stats.pending > 0 ? "border-red-200 dark:border-red-800" : "border-border/50"}`}
              >
        ...
        {pendingOrders.length > 0 && filterStatus === "all" && (
          <Card className="border-red-200 dark:border-red-800 shadow-sm bg-red-50 dark:bg-red-900/10 mb-6">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <AlertCircle className="w-4 h-4 text-red-600" />
                <p className="text-sm font-semibold text-red-700 dark:text-red-400">
                  {pendingOrders.length} order
                  {pendingOrders.length !== 1 ? "s" : ""} requiring your action
                </p>
              </div>
                ...
                {pendingOrders.slice(0, 3).map((order: any) => (
                  ...
                  >
                    ...
                    <Link href={`/seller/orders/${order.id}`}>
                      <Button
                        size="sm"
                        className="h-7 text-xs px-3 ml-3"
                      >
                        Review
                      </Button>
                    </Link>
        ...
        {allList.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide mb-4">
            {[
              ...
              <Link
                key={tab.id}
                href={`/seller/orders${tab.id === "all" ? "" : `?status=${tab.id}`}`}
              >
                <Badge
          ...
          {orderList.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3 shrink-0 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      <Link href={`/seller/orders/${order.id}`}>
                        <Button size="sm" variant="outline" className="h-8 text-xs">
                          {order.status === "pending" ? "Review" : "Details"}
            ...
            <div className="text-center py-16 bg-card rounded-lg border border-dashed border-border/50">
              ...
              <p className="text-muted-foreground mb-6 max-w-md mx-auto text-sm">
                Orders will appear here when buyers purchase your products or
                accept your offers.
              </p>
              <Link href="/seller/explore">
                <Button size="sm">Browse Buyer Requests</Button>
              </Link>
            </div>
          )}
        </div>
