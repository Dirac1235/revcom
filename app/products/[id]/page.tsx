...
export default function ProductDetailPage() {
  ...
  const inventoryStatus = product ? getInventoryStatus() : { status: 'in_stock', quantity: 0 };

  const getStatusBadge = () => {
    const badgeConfig: Record<string, { color: string; label: string }> = {
      in_stock: { color: 'bg-emerald-50 text-emerald-700 border-emerald-200 shadow-sm', label: `${inventoryStatus.quantity} units available` },
      low_stock: { color: 'bg-amber-50 text-amber-700 border-amber-200 shadow-sm', label: `Low stock (${inventoryStatus.quantity} left)` },
      out_of_stock: { color: 'bg-red-50 text-red-700 border-red-200 shadow-sm', label: 'Out of stock' },
    };
  ...
  const canOrder = !isOwnProduct && product?.status === 'active' && inventoryStatus.status !== 'out_of_stock';
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Navigation */}
        <div className="mb-8 flex items-center justify-between animate-fadeIn" style={{ animationDelay: '0.05s' }}>
          <Link href={ROUTES.PRODUCTS}>
            <Button 
              variant="ghost" 
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="md:col-span-2 space-y-6 lg:space-y-8">
            ...
            <div className="space-y-3 animate-fadeIn" style={{ animationDelay: '0.1s' }}>
              {/* Main Image */}
              <div className="aspect-square rounded-xl overflow-hidden bg-gradient-to-br from-secondary/20 to-secondary/5 border border-border relative group shadow-md hover:shadow-lg transition-shadow duration-500">
                <img
                  src={selectedImage}
                  alt={product.title}
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 snap-x snap-mandatory">
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-300 snap-center ${
            ...
            <div className="space-y-5 animate-fadeIn" style={{ animationDelay: '0.15s' }}>
              ...
              <div className="space-y-3 py-4 border-y border-border/30">
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-lg text-muted-foreground font-medium pb-1">ETB</span>
                </div>
                <Badge className={`${statusBadge.color} border px-3 py-1.5 text-xs font-semibold rounded-full w-fit`}>
                  {statusBadge.label}
                </Badge>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-secondary/20 border border-border/30 backdrop-blur-sm">
                  <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide mb-1.5">In Stock</p>
                  <p className="text-2xl font-bold text-foreground">{product.inventory_quantity}</p>
                </div>
                <div className="p-3 rounded-lg bg-secondary/20 border border-border/30 backdrop-blur-sm">
                  <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide mb-1.5">Views</p>
            ...
            <div className="space-y-4 animate-fadeIn" style={{ animationDelay: '0.2s' }}>
              <div className="border-l-4 border-primary pl-4 py-4">
                <h3 className="text-base font-semibold text-foreground mb-3">About this product</h3>
            ...
            {product.specifications && Object.keys(product.specifications).length > 0 && (
              <div className="space-y-4 animate-fadeIn" style={{ animationDelay: '0.25s' }}>
                <div>
                  <h3 className="text-base font-semibold text-foreground mb-4">Specifications</h3>
          ...
          <div className="space-y-5 lg:space-y-6 md:sticky md:top-6 md:h-fit">
            ...
            {/* Seller Card */}
            <div 
              className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-secondary/5 backdrop-blur-sm p-4 hover:border-border/50 transition-all duration-300 shadow-sm hover:shadow-md animate-fadeIn"
              style={{ animationDelay: '0.15s' }}
            >
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-4">Seller Info</h3>
              ...
              <div className="flex items-start gap-3 mb-4">
                {seller?.avatar_url ? (
                  ...
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 border-2 border-primary/20 flex items-center justify-center shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {seller?.user_type && (
                    <div className="flex items-center gap-1 mt-2 text-emerald-600">
                      ...
                      <span className="text-xs font-semibold capitalize">{seller.user_type.replace('_', ' ')}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Order Panel */}
            <div 
              className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-secondary/5 backdrop-blur-sm p-4 space-y-4 shadow-sm hover:shadow-md transition-all duration-300 animate-fadeIn"
            ...
            >
                ...
                {canOrder && (
                  <div className="flex items-center gap-3">
                    <div className="flex items-center border-2 border-border/30 rounded-lg p-1.5 bg-secondary/20 backdrop-blur-sm">
                      ...
                      <button
                        onClick={() => setQuantity(Math.min(inventoryStatus.quantity, quantity + 1))}
                    ...
                    <span className="text-xs text-muted-foreground font-medium">Max: {inventoryStatus.quantity}</span>
                  </div>
                )}
              </div>
              ...
              <div className="py-3.5 border-t border-b border-border/30">
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2">Total</p>
                ...
                {!isOwnProduct && product.status === 'active' ? (
                  <>
                    <Button
                      ...
                      disabled={inventoryStatus.status === 'out_of_stock' || isCreatingConversation}
                    ...
                    >
                      ...
                      {inventoryStatus.status === 'out_of_stock' ? 'Out of Stock' : 'Place Order'}
                    </Button>
                    <Button
                  ...
                  <Link href={ROUTES.SELLER_PRODUCT_EDIT(product.id)} className="w-full">
                    <Button 
                      variant="outline" 
                      className="w-full border-2 border-border/30 hover:border-primary/30 hover:bg-secondary/20 text-foreground h-10 text-sm font-semibold rounded-lg transition-all duration-300"
                    >
            ...
            {/* Delivery Info */}
            <div 
              className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-secondary/5 backdrop-blur-sm p-4 space-y-3 shadow-sm hover:shadow-md transition-all duration-300 animate-fadeIn"
              style={{ animationDelay: '0.25s' }}
            >
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Shipping</h3>
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-secondary/20 backdrop-blur-sm">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide mb-0.5">Delivery</p>
                    <p className="text-xs font-semibold text-foreground">3-5 business days</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-secondary/20 backdrop-blur-sm">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide mb-0.5">Cost</p>
                    <p className="text-xs font-semibold text-foreground">Free over 5,000 ETB</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Trust Signals */}
            <div 
              className="rounded-xl border border-border/30 bg-gradient-to-br from-background to-secondary/5 backdrop-blur-sm p-4 space-y-2.5 shadow-sm hover:shadow-md transition-all duration-300 animate-fadeIn"
              style={{ animationDelay: '0.3s' }}
            >
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Why Buy Here</h3>
              
              <div className="space-y-2">
                <div className="flex items-center gap-2.5 p-2.5 rounded-lg bg-secondary/10">
                  <Lock className="w-4 h-4 text-primary flex-shrink-0" />
                  <span className="text-xs text-foreground">Secure messaging</span>
                </div>
                <div className="flex items-center gap-2.5 p-2.5 rounded-lg bg-secondary/10">
                  <Shield className="w-4 h-4 text-primary flex-shrink-0" />
                  <span className="text-xs text-foreground">Buyer protection</span>
                </div>
                <div className="flex items-center gap-2.5 p-2.5 rounded-lg bg-secondary/10">
                  <CheckCircle className="w-4 h-4 text-primary flex-shrink-0" />
                  <span className="text-xs text-foreground">Verified seller</span>
                </div>
              </div>
            </div>
          </div>
